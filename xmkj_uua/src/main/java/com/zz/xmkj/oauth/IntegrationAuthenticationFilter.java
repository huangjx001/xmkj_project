...
{
    private static final String AUTH_TYPE_PARM_NAME = "grant_type";// 登录类型参数名
        ...
        ServletException
    {
        ...
        if (requestMatcher.matches(request))
        {
            ...
            if (StringUtils.isEmpty(requestParameterWrapper.getParameter("password")))
            {
    ...
    private void prepare(IntegrationAuthenticationEntity entity)
    {
        if (entity != null)
        {
            synchronized (this)
            {
                Map<String, IntegrationAuthenticator> map = applicationContext.getBeansOfType(
                    IntegrationAuthenticator.class);
                if (map != null)
                {
    ...
    /**
     * 用途：在拦截时给Request添加参数 Cloud OAuth2 密码模式需要判断Request是否存在password参数， 如果不存在会抛异常结束认证
