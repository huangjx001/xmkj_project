...
/**
 * 〈重写RedisTokenStore〉 spring5.0修改了一些方法，导致不兼容，需要重写全部的set()为stringCommands().set()
 * 
 * @author huangjx
 * @since 1.0.0
 */
public class RedisTokenStore implements TokenStore
{
    ...
    {
        return (OAuth2AccessToken)this.serializationStrategy.deserialize(bytes,
            OAuth2AccessToken.class);
    }

    private OAuth2Authentication deserializeAuthentication(byte[] bytes)
    {
        return (OAuth2Authentication)this.serializationStrategy.deserialize(bytes,
            OAuth2Authentication.class);
    }

    private OAuth2RefreshToken deserializeRefreshToken(byte[] bytes)
    {
        return (OAuth2RefreshToken)this.serializationStrategy.deserialize(bytes,
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication)
    {
        ...
        if (accessToken != null)
        {
            OAuth2Authentication storedAuthentication = this.readAuthentication(
                                  ...
                                  OAuth2Authentication authentication)
    {
        ...
        try
        {
            ...
            if (refreshToken instanceof ExpiringOAuth2RefreshToken)
            {
                ...
                {
                    int seconds = Long.valueOf(
                        (expiration.getTime() - System.currentTimeMillis()) / 1000L).intValue();
